...
const startVoiceDetection = () => {
  ...
  recognition.onresult = (event: any) => {
    ...
    const panicWords = ["help", "emergency", "danger", "save me"];

    const detected = panicWords.some(word =>
    ...
    if (detected && state === "idle") {
      ...
      toast({
        title: "Emergency Voice Detected",
...
// VOICE DETECTION (separate hook)
useEffect(() => {
  console.log("Voice detection started");
  startVoiceDetection();
}, []);

  // Fetch location + send alerts
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        const BOT_TOKEN = "8747408621:AAEd57AD0okWprU128nkrlzjDwZS_4SDtME";
        const CHAT_ID = "-1003517170218";

        const sendTelegramAlert = async () => {
          const message = `
🚨 DOCSHIELD EMERGENCY ALERT

Doctor needs immediate help!
...
`;
        ...
        contacts?.forEach((contact) => {
          console.log(
            `Alert sent to contact: ${contact.name} (${contact.phone})`
          );
        });
      ...
      () => {
        toast({
          ...
          description:
            "Could not fetch GPS location. Please enable location services.",
          variant: "destructive",
        });
        resetState();
      },
      ...
      <div className="flex flex-col items-center gap-6 p-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            {contactCount} contact{contactCount !== 1 ? "s" : ""} notified with
            your location
          </p>
        </div>
      ...
      <div className="flex flex-col items-center gap-6 p-6">
        <div className="relative w-36 h-36 flex items-center justify-center">
          ...
          <div className="relative flex flex-col items-center">
            <span className="text-6xl font-bold text-emergency">
              {countdown}
            </span>
  ...
  return (
    <div className="flex flex-col items-center gap-6 p-6">
      ...
      <p className="text-sm text-muted-foreground text-center max-w-[240px]">
        Press the SOS button in case of emergency. Your GPS location will be
        sent to all registered contacts.
      </p>
    </div>
  );
};
